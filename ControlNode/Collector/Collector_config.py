...
class Collector_config:
	...
	def __init__(self, exefpath):
                ...
                Collector_config.home_dir = os.path.split(os.path.dirname(os.path.realpath(exefpath)))[0]
		#Load the configuration file
		config = ConfigParser(CFG_DEFAULTS)
		#Assumes file exists in workdir
		config.read(Collector_config.home_dir + '/cfg/clen_collector.cfg')
		#Setup the values
	...
	def get_home_dir(self):
		return(Collector_config.home_dir)
        ...
        def _load_transcalibration(self) :
                ...
                f = open(Collector_config.home_dir + '/cfg/clen_transcalibration.cfg')
                while True:
                        fline=f.readline()
                        if fline == '':
                                break
